...
var _makeArray = function (o) {
    ...
    if (typeof o.length !== 'number' || _.isString(o) || _.isFunction(o)) {
        return [o];
    }
    var ret = [];
...
var augment = function (/*r, s1, s2, ..., ov, wl*/) {
...
var isLocalFile = function(uri, debug){
    //过滤时间戳
    uri = uri.replace(/\?.*/, '');
    if(process.platform == 'win32') {
        ...
        if(fs.existsSync(path.resolve(uri))) {
            ...
            if(debug) {
                console.log('WARN:%s is not found', uri);
            }
        }

        return uri.indexOf('/home') == 0 || uri.indexOf('/Users') == 0;
    }
};

var createSnapGuid = function(uri){
    var stamp = new Date().getTime();
    return new Buffer([uri, stamp].join('_')).toString('base64');
};
 ...
 * @returns {{}}
 */
var getTemplateSetVars = function(s){
    var result = {};
    if(s) {
        var keys = s.match(/#(set|SET)\s*\(.*?\)/g);
        _.each(keys, function(v){
            var vars = valueParser.getVars(v), t = {};
            if(vars.length && vars.length > 1) {
                ...
                result[vars[0]] = v.indexOf('control/') == -1 ? 'control/' + v : v;
            }
        });
    }
...
var anyalyzeAppType = function(p){
    var lists = fs.readdirSync(path.resolve(p));

    if(_.some(lists, function(filename) {
        return /(pom.xml)|(WEB-INF)|(screen)|(.jsp)/.test(filename);
    })) {
        return 'webx';
    } else {
        return 'php';
...
module.exports = {
    ...
    createSnapGuid: createSnapGuid,
    getTemplateSetVars: getTemplateSetVars,
    trim: trim,
    newGuid: newGuid,
    isWindows: function(){
        return process.platform == 'win32';
    },
    anyalyzeAppType: anyalyzeAppType,
    isAssetsUrl: function(path) {
        return /.*\.(css|js|ico|png|jpg|swf|less|gif|woff|scss|eot|ttf|svg).*/.test(path);
    }
};