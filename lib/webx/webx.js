...
module.exports = {
    ...
    getControlFormat: function(controlPath, modules, commonModules, callback){
        ...
        formatControlPart.replace(/^\/?control/, '');


        if(_.indexOf(_.keys(commonModules), formatControlPart.split('/')[0]) == -1) {
            _.each(modules, function(moduleConfig, moduleName) {
                if(!hasModule || (hasModule && moduleName == searchModule)) {
                    ...
                    if(fs.existsSync(searchBasePath)) {
                        ...
                        if(fs.existsSync(searchFullPath)) {
                            callback(moduleName + ':' + formatControlPart);
                            return;
                        }
                    }
                }
