...
module.exports = {
    findWebroot: function(p, type, callback){
        if(type == 'b2b') {
            b2bFinder.findWebroot(p, function(err, result) {
                if(err) {
                    callback(err);
                }
                if(result.length > 0) {
            ...
            tbFinder.findWebroot(p, function(err, result) {
                if(err) {
                    callback(err);
                }
                if(result.length > 0) {
...
var getLayout = function(p, baseModule, config, finder, screenContent){
    ...
    //首先在模板中查找是否有指定的layout
    if(result && result.length) {
        layout = result[0].replace(/.*setLayoutTemplate\((\s)*('|")?\s*/, '');
    ...
    result = c.match(/\$\w+\.setLayout\((null|"[^"]*"|'[^']*')\)/g);
    if(result && result.length) {
        layout = result[0].replace(/.*setLayout\((\s)*('|")?\s*/, '');
        layout = layout.replace(/('|")?\s*\)/, '');
        if(layout == 'null') return layout;
        layout = layout.replace(/layout(\/)?/, '');
        p = 'screen/' + layout;
    ...
    //再查找是否是java中指定，在这里表现为用户配置
    var layoutSets = config['layout'];
    if(layoutSets && layoutSets[p]) {
        if(layoutSets[p] == 'null') {
            return layoutSets[p];
        }
        var split = layoutSets[p].split(':');
        return finder.getLayout('screen/' + split[1], split[0], config);
    }
