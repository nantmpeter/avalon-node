...
module.exports = {
    ...
    getFile: function(p, innerModule, config) {
        if(config['subModule'][innerModule]) {
            ...
            var vmPath = pathPrefix ? path.join(pathPrefix, t.join(path.sep)) : '';
    ...
    getLayout: function(p, baseModule, config) {
        ...
        if(fs.existsSync(vmPath)) {
            ...
            while(p.indexOf('/') != -1) {
                ...
                if(fs.existsSync(vmPath)) {
                    p = path.join(p, 'default.vm').replace(/\\/g, '/');
                    return p;
                } else {
                    //一层层查找
                    vmPath= path.resolve(path.dirname(vmPath), '..');
                    p = path.dirname(p);
                }
            }
